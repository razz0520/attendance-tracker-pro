...
const App = () => {
  ...
  // --- Auth & Data Sync Hook ---
  useEffect(() => {
    // Detect if user is coming from a password reset email
    ...
    <div className={`min-h-screen transition-colors duration-300 ${isDarkMode ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'} pb-24 md:pb-0 md:pl-20`}>
                  ...
                  <div key={subject.id} className={`${isDarkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-white border-slate-100'} border p-6 rounded-[2rem] shadow-sm hover:shadow-xl transition-all group ${isCritical ? 'shadow-red-500/20 border-red-500/50' : ''}`}>
                    ...
                    <div className="mb-6">
                      ...
                      <div className="h-2 w-full bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
                        <div className={`h-full transition-all duration-1000 ${isCritical ? 'bg-red-500' : 'bg-emerald-500'}`} style={{ width: `${Math.min(percentage, 100)}%` }} />
                      </div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <button onClick={() => markAttendance(subject.id, 'p')} className="flex flex-col items-center gap-1 p-3 rounded-2xl bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 hover:bg-emerald-600 hover:text-white transition-all">
                        <Check size={20} /><span className="text-[10px] font-bold uppercase">Present</span>
                      </button>
                      <button onClick={() => markAttendance(subject.id, 'a')} className="flex flex-col items-center gap-1 p-3 rounded-2xl bg-red-50 dark:bg-red-900/20 text-red-500 hover:bg-red-500 hover:text-white transition-all">
                        <X size={20} /><span className="text-[10px] font-bold uppercase">Absent</span>
                      </button>
                      <button onClick={() => undoLast(subject)} className="flex flex-col items-center gap-1 p-3 rounded-2xl bg-slate-100 dark:bg-slate-800 text-slate-500 hover:bg-slate-600 hover:text-white transition-all">
                        <RotateCcw size={18} /><span className="text-[10px] font-bold uppercase">Undo</span>
                      </button>
                    </div>
                  </div>
                );
          ...
          <div className="max-w-xl mx-auto space-y-6">
            ...
            <div className={`${isDarkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-white border-slate-100'} border rounded-[2rem] overflow-hidden`}>
              <div className="p-6 border-b border-slate-800/10 flex items-center justify-between">
                ...
                <button onClick={handleLogout} className="p-3 text-red-500 hover:bg-red-50 rounded-xl transition-all"><LogOut size={20} /></button>
              </div>
              <div className="p-6 flex items-center justify-between">
                <div><h4 className="font-bold">Export Data</h4><p className="text-sm text-slate-500">Download .json backup</p></div>
                <button onClick={() => {
                ...
                }} className="p-3 bg-slate-100 dark:bg-slate-800 rounded-xl hover:bg-indigo-600 hover:text-white transition-all"><Download size={20} /></button>
              </div>
            </div>
          </div>
        )}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm">
          <div className={`${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'} w-full max-w-md border rounded-[2.5rem] p-8 shadow-2xl`}>
            <h3 className="text-2xl font-black mb-6">New Subject</h3>
            <div className="space-y-6">
              <div>
                <label className="block text-xs font-bold uppercase tracking-widest mb-2 opacity-50">Name</label>
                <input type="text" value={newSub.name} onChange={e => setNewSub({ ...newSub, name: e.target.value })} className={`w-full p-4 rounded-2xl border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-slate-50 border-slate-200'} outline-none focus:ring-2 focus:ring-indigo-500`} />
              </div>
              <div>
                <label className="block text-xs font-bold uppercase tracking-widest mb-2 opacity-50">Target: {newSub.target}%</label>
                <input type="range" min="50" max="100" step="5" value={newSub.target} onChange={e => setNewSub({ ...newSub, target: parseInt(e.target.value) })} className="w-full accent-indigo-600" />
              </div>
